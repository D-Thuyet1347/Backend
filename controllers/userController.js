...
const updateUser = async (req, res) => {
    try {
        const updateUser = await blogModel.findByIdAndUpdate(req.params.id, req.body, { new: true });
        if (!updateUser) {
  ...
  const updateUserRole = async (req, res) => {
    const { id } = req.params;
    const { firstName, lastName, phone, address, email, dateOfBirth,role } = req.body;  
    try {
